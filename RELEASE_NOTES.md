...
- **阅读器目录/设置页改为独立页面**：目录和设置不再使用底部面板，改为常规全屏页面，支持顶部返回按钮和系统返回手势。
- **目录页间距优化**：章节搜索框与章节列表之间增加留白，避免视觉拥挤和误触。
- **滚动到底修复**：目录列表和设置页面增加底部安全留白，避免底部选项被版式/系统栏遮挡导致无法滑到底。
- **默认阅读边距调整**：用户设置页边距为 0 时，左右实际基准边距从 8dp 调整为 16dp，正文不再贴边。

## 版本

- 应用版本：1.2.4，Android versionCode 为 45。

---

Visto 1.2.3

- **目录页体验优化**：目录呼出改为非拖拽底部面板，颜色统一走 Material 主题；列表独立滚动不再带动整个面板移动。
- **书籍编码全面增强**：新增 Big5（繁体中文）、EUC-JP（日文）、Shift_JIS（日文）、EUC-KR（韩文）、GB18030 编码支持。编码检测从仅 GBK/UTF-8 扩展到 6 种 CJK 编码，采用字节对 + Unicode 文字特征双轨评分精准识别。
- **加载韧性提升**：新增两轮解码策略——优先严格解码保证完整性，失败后宽松解码容忍少量损坏字节，同时确保控制字符和替换字符不超过 1% 阈值。即使下载文件有少量缺陷也能正常阅读。
- **编码回退链扩展**：解码候选列表新增 10 种编码回退（UTF-8、GBK、GB18030、Big5、EUC-JP、EUC-KR、Shift_JIS、windows-1252、ISO-8859-1），最大限度适配各类文本文件。

## 版本

- 应用版本：1.2.3，Android versionCode 为 44。

---

Visto 1.2.2

- **双击缩放修复**：双击放大后再次双击无法复原的问题已修复。用显式缩放状态追踪替代浮点比较，消除手势检测器抢事件导致的失效。

## 版本

- 应用版本：1.2.2，Android versionCode 为 43。

---

Visto 1.2.1

- **滚动位置保留**：文件夹列表 ↔ 详情页、书架 ↔ 阅读器之间往返不再丢失滚动位置。

## 版本

- 应用版本：1.2.1，Android versionCode 为 42。
...
- 重新设计阅读器页边距：滑块从 0 开始，0 表示基准间距（上 12dp / 下 52dp / 左右 16dp），滑块值在基准之上追加。底部基准由胶囊底间隙(12dp) + 胶囊高度(≈28dp) + 正文与胶囊间隙(12dp) 构成。
