...
- **阅读失败恢复优化**：阅读加载失败页新增“清理本书缓存并重试”，只清理当前书籍的本地文本缓存，不删除阅读进度，也不会修改 WebDAV 远端文件。
- **目录定位优化**：打开目录页时自动定位到当前章节，同时保留当前章节高亮；搜索章节时不会被强制拉回当前章节。
- **版本文档同步**：README / README.en 当前版本同步到 1.2.7。
- 应用版本：1.2.7，Android versionCode 为 48。

---

Visto 1.2.6

- 修复阅读进度恢复时偶尔显示章节首页、下一页却跳到最后阅读页后一页的问题；阅读页现在统一以会话页码为唯一来源，避免 UI 页码与保存进度状态不同步。
- 增加阅读进度恢复回归测试，覆盖 `pageOffset` 与 `pageStartChar` 不一致时应按真实页锚点恢复。
- 应用版本：1.2.6，Android versionCode 为 47。

Visto 1.2.5

- **章节目录识别优化**：章节识别改为按行匹配，只把独立成行的章节标题识别为目录，避免正文里的“这是我第一回去北京”“他在第一章里写下这句话”等句子误入目录。
- **旧阅读进度兼容**：目录规则变化后，打开旧书籍时优先用已保存的全文字符位置恢复阅读点，减少旧 chapterIndex 因目录重算而跳错章节的情况。
- **目录页精简**：目录列表去掉软件自动生成的“第 N 章”副标题，只保留文档中真实识别出的章节标题。
- **目录页间距优化**：章节列表改为更紧凑的单行布局，缩小章节之间的垂直间距。

## 版本

- 应用版本：1.2.5，Android versionCode 为 46。
