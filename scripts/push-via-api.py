...
This is Visto's durable workaround for this workstation, where normal
`git push`/`git fetch` over HTTPS can time out even though `gh api` works.

Default behavior:
- Pushes local commits to Vivitoto/Visto main via GitHub's Git Data API.
...
DEFAULT_REPO = os.environ.get("VISTO_REPO", "Vivitoto/Visto")
DEFAULT_BRANCH = os.environ.get("VISTO_BRANCH", "main")
